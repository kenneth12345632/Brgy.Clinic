    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8 bg-slate-800 text-white flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="bg-slate-700/50 p-4 rounded-2xl border border-slate-600 w-full md:w-auto">
                <p class="text-xs uppercase font-bold text-slate-400 mb-1">Assigned Service</p>
        ...
        <div class="p-8">
            <h2 class="text-lg font-bold text-slate-800 mb-8 flex items-center gap-2">
                <i class="fa-solid fa-clock-rotate-left text-blue-500"></i> Clinical History
            </h2>
                ...
                <div class="relative pl-12 pb-6">
                    ...
                    <div class="bg-slate-50 border border-slate-100 rounded-2xl p-6 shadow-sm">
                        <div class="flex flex-col md:flex-row justify-between items-start mb-4 gap-2">
                            <div>
                                <span class="bg-blue-100 text-blue-700 text-[10px] font-black px-2 py-1 rounded uppercase tracking-tighter">Registration</span>
                                <h4 class="text-lg font-bold text-slate-800 mt-2">Initial Inquiry: {{ $patient->service }}</h4>
                            </div>
                        ...
                        <div class="text-slate-600 space-y-3 text-sm leading-relaxed">
                            <p>Patient <strong>{{ $patient->first_name }}</strong> registered for <strong>{{ $patient->service }}</strong>. The address recorded is <strong>{{ $patient->address }}</strong>.</p>
                            <div class="bg-white p-3 rounded-lg border border-slate-200 italic text-slate-500 text-xs">
                                Note: This record was created on {{ $patient->created_at->format('M d, Y') }}.
                            </div>
                        </div>
                    </div>
                </div>
