    ...
    <div class="mb-8">
        ...
        <div class="mt-4 flex justify-between items-end">
            ...
            <div class="bg-blue-50 px-6 py-3 rounded-2xl border border-blue-100 text-right">
                ...
                <p class="text-2xl font-black text-blue-800">{{ $patientCount }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    {{-- Changed from $patients to $appointments to match the Controller's bridge variable --}}
                    @forelse($appointments as $patient)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4 font-bold text-slate-800">{{ $patient->name }}</td>
                            <td class="px-6 py-4 text-slate-600 text-center">{{ $patient->age }}</td>
                            <td class="px-6 py-4 text-slate-600">
                                <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase {{ $patient->gender == 'Male' ? 'bg-blue-50 text-blue-600' : 'bg-pink-50 text-pink-600' }}">
                                    {{ $patient->gender }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-slate-500">{{ $patient->address }}</td>
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-slate-400 font-medium">No patients found for this service.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
